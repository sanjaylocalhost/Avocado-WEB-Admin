...
export default function Login() {
  const { login, adminLogin, admin, user, loading } = useAuth();
  ...
  const [shouldRedirect, setShouldRedirect] = useState(false); // 👈 Control redirect

  // Only redirect after successful login submission, not on initial load
  useEffect(() => {
    if (loading) return;
    
    // Only redirect if shouldRedirect is true (meaning user just logged in)
    if (shouldRedirect) {
      if (loginType === "admin" && admin) {
        console.log("✅ Admin logged in, redirecting to dashboard");
        navigate("/admin/dashboard", { replace: true });
      } else if (loginType === "user" && user) {
        console.log("✅ User logged in, redirecting to dashboard");
        const redirectTo = location.state?.from?.pathname || "/dashboard";
        navigate(redirectTo, { replace: true });
      }
    }
  }, [admin, user, loading, navigate, loginType, location, shouldRedirect]);

  // Reset shouldRedirect when loginType changes
  useEffect(() => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (loginType === "admin") {
        ...
        // Set shouldRedirect to true AFTER successful login
